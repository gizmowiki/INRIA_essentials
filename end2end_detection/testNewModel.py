...
def processImage(img_path, img, model, eng, image_enhancement, nms_threshold, predict_threshold, ed_bx_threshold,
                 img_shape, annotation_list, max_overlap):
...
def show(filelist_path, video, base_out_path, predict_threshold, ed_bx_threshold,
    ...
    weights_path = '/home/rpandey/people_detect/weights_fit_negatives_nostalgia/weights-improvement-59-0.653.hdf5'

    start_time = time.time()
    model = getModelNewFitNoFitNostalgia(weights_path, img_shape)
    print("Loaded keras model and weights with time taken ", str(time.time() - start_time), "seconds")

    if not video:
        for img_path in files:
            img = cv2.imread(img_path, 2)
            img_detect, result_data = processImage(img_path, img=img, model=model, eng=eng,
...
class TestData:
    def __init__(self, filelist_path, annotation_dict, base_out_path, videos=False,
                 predict_threshold=0.999, nms_threshold=0.5, ed_bx_threshold=0.7, max_overlap=0.5,
